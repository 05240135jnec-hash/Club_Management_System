...
export default function StudentCertificate() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/certificate/student/${clubId}`, { headers: { Authorization: `Bearer ${getToken()}` } })
      .then(r => r.json())
  ...
  async function drawSignatures(ctx, W, H, certData) {
    ...
    if (certData.super_admin_signature) {
      ...
      if (img2) {
        ctx.drawImage(img2, W * 0.670, H * 0.860 - sigHeight, sigWidth, sigHeight);
      }
    }
  }
  ...
  function overlayData(ctx, W, H, certData) {
    ...
    ctx.fillText(certData.club_name + ' Member',   W * 0.400, H * 0.635);
  ...
  if (loading) return (
    <StudentLayout pageTitle="Certificate" pageSubtitle="Generate your club membership certificate">
  ...
  if (!clubId) return (
    <StudentLayout pageTitle="Certificate" pageSubtitle="Generate your club membership certificate">
      <div style={{ textAlign: 'center', padding: 80, color: '#64748b' }}>No club selected.</div>
    </StudentLayout>
  );
  ...
  const requirements = data ? [
    ...
    { label: `Year 2 or Year 4 graduating (Your year: ${data.year})`,                                                      met: data.is_graduating },
    { label: `Attendance 90%+ (Your attendance: ${data.attendance_pct}%)`,                                                 met: data.attendance_pct >= 90 },
    { label: `Minimum ${data.min_sessions ?? 5} sessions required (Your club has: ${data.total_sessions ?? 0} sessions)`, met: data.club_has_enough_sessions },
          ...
          <div style={{ position: 'relative' }} ref={reqRef}>
            ...
            {showReqs && (
              <div style={{
                ...
                zIndex: 999, minWidth: 340, padding: '16px 18px',
              }}>
