...
import '../../styles/advisor/upload_image.css';

const API = 'http://127.0.0.1:8000/api';
function getToken() { return localStorage.getItem('token'); }

export default function AdvisorUploadImage() {
  ...
  useEffect(() => {
    fetchAlbums();
    const handler = e => { if (!e.target.closest('.img-menu-wrap')) setOpenMenuId(null); };
  ...
  async function fetchAlbums() {
    setLoading(true);
    try {
      const res  = await fetch(`${API}/image-albums`, { headers: { Authorization: `Bearer ${getToken()}` } });
  ...
  function handleFiles(files) {
    ...
    imgs.forEach(f => { if (!merged.find(x => x.name === f.name && x.size === f.size)) merged.push(f); });
    if (merged.length > 10) {
  ...
  function buildPreviews(files, setter) {
    ...
    files.forEach((file, i) => {
      ...
      reader.onload = e => { results[i] = e.target.result; done++; if (done === files.length) setter([...results]); };
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const res  = await fetch(`${API}/image-albums`, { method: 'POST', headers: { Authorization: `Bearer ${getToken()}` }, body: fd });
      const data = await res.json();
      if (res.ok) {
  ...
  async function handleDeleteAlbum(id, albumTitle) {
    const result = await Swal.fire({
      title: 'Delete Album?',
      html: `Delete <strong>${albumTitle}</strong> and all its images?<br>This cannot be undone.`,
    ...
    try {
      const res = await fetch(`${API}/image-albums/${id}`, { method: 'DELETE', headers: { Authorization: `Bearer ${getToken()}` } });
      if (res.ok) { Swal.fire({ icon: 'success', title: 'Deleted!', timer: 1500, showConfirmButton: false }); fetchAlbums(); }
    } catch {}
  }
  ...
  function handleEditFiles(files) {
    ...
    imgs.forEach(f => { if (!merged.find(x => x.name === f.name && x.size === f.size)) merged.push(f); });
    const existingCount = editAlbum ? editAlbum.images.filter(img => !editRemoveIds.includes(img.id)).length : 0;
    const total = existingCount + merged.length;
    if (total > 10) {
      Swal.fire({ icon: 'warning', title: 'Maximum 10 images', text: 'Total images in album cannot exceed 10.' });
  ...
  async function handleEditSave() {
    ...
    const remainingImages = editAlbum.images.filter(img => !editRemoveIds.includes(img.id));
    if (remainingImages.length + editNewFiles.length === 0) {
    ...
    try {
      ...
      const res  = await fetch(`${API}/image-albums/${editAlbum.id}`, { method: 'POST', headers: { Authorization: `Bearer ${getToken()}` }, body: fd });
      const data = await res.json();
      if (res.ok) {
  ...
  function openLightbox(images, index, albumTitle) {
    setLightbox({ images, index, title: albumTitle });
  }

  function lightboxPrev() { setLightbox(lb => ({ ...lb, index: Math.max(0, lb.index - 1) })); }
  function lightboxNext() { setLightbox(lb => ({ ...lb, index: Math.min(lb.images.length - 1, lb.index + 1) })); }
  ...
  function getFiltered() {
    ...
    if (search.trim()) {
      ...
      result = result.filter(a => a.title.toLowerCase().includes(q) || a.caption?.toLowerCase().includes(q));
    }
  ...
  const canUpload = title.trim() && caption.trim() && selectedFiles.length > 0;

  return (
    <AdvisorLayout title="Upload Image" subtitle="Upload club images">
        ...
        <div className="img-upload-card">
          ...
          <div className="img-form-row">
            <div className="img-form-group">
              ...
              <input className="img-input" type="text" value={title} onChange={e => setTitle(e.target.value)} maxLength={80} />
            </div>
            <div className="img-form-group">
              <label className="img-label">Caption <span className="img-required">*</span></label>
              <textarea className="img-textarea" value={caption} onChange={e => setCaption(e.target.value)} maxLength={240} rows={2} />
            </div>
          </div>
          ...
          >
            ...
            <div className="img-drop-text">Click or drag &amp; drop images here <span className="img-required">*</span></div>
              ...
              <div className="img-preview-grid">
                {previews.map((src, i) => (
                  <div key={i} className="img-preview-item">
                    <img src={src} alt="" />
                    <button className="img-preview-remove" onClick={e => { e.stopPropagation(); removeFile(i); }}>✕</button>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="img-upload-btn-row">
            <button className="img-submit-btn" onClick={handleUpload} disabled={!canUpload || uploading}>
        ...
        <div className="img-gallery-card">
          <div className="img-gallery-header">
            ...
            <div className="img-gallery-controls">
              <div className="img-search-wrap">
                ...
                <input className="img-search" type="text" value={search} onChange={e => setSearch(e.target.value)} />
              </div>
          ...
          {loading ? (
            ...
            <div className="img-empty">
              ...
              <p className="img-empty-hint">{albums.length === 0 ? 'Upload images above and they\'ll appear here as albums.' : 'Try a different search term.'}</p>
            </div>
          ) : (
            <div className="img-album-grid">
              {filtered.map(album => (
                <div key={album.id} className="img-album-card">
                  <div className="img-album-thumb" onClick={() => openLightbox(album.images.map(i => i.url), 0, album.title)}>
                  ...
                  <div className="img-album-info">
                    <div className="img-album-info-row">
                      <div className="img-album-title">{album.title}</div>
                      <div className="img-menu-wrap" onClick={e => e.stopPropagation()}>
                        <button className="img-menu-btn" onClick={() => setOpenMenuId(openMenuId === album.id ? null : album.id)}>
                        ...
                        {openMenuId === album.id && (
                          <div className="img-menu-dropdown">
                            ...
                            <button className="img-menu-item delete" onClick={() => handleDeleteAlbum(album.id, album.title)}>
                              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="13" height="13">
                    ...
                    <div className="img-album-caption">{album.caption}</div>
                    <div className="img-album-meta">
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="11" height="11">
          ...
          <div className="img-lightbox" onClick={() => setLightbox(null)}>
            <div className="img-lightbox-panel" onClick={e => e.stopPropagation()}>
              ...
              <div className="img-lb-img-wrap">
                {lightbox.index > 0 && (
                  <button className="img-lb-arrow left" onClick={lightboxPrev}>
                ...
                <img className="img-lb-img" src={lightbox.images[lightbox.index]} alt="" />
                {lightbox.index < lightbox.images.length - 1 && (
                  <button className="img-lb-arrow right" onClick={lightboxNext}>
              ...
              <div className="img-lb-footer">
                <div>
                  <div className="img-lb-title">{lightbox.title}</div>
                  <div className="img-lb-counter">{lightbox.index + 1} / {lightbox.images.length}</div>
                </div>
                <div className="img-lb-dots">
                  {lightbox.images.length <= 20 && lightbox.images.map((_, i) => (
                    <button key={i} className={`img-lb-dot${i === lightbox.index ? ' active' : ''}`}
          ...
          <div className="img-edit-backdrop" onClick={() => setEditAlbum(null)}>
            <div className="img-edit-modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="img-edit-body">
                <div className="img-form-group">
                  <label className="img-label">Title <span className="img-required">*</span></label>
                  <input className="img-input" type="text" value={editTitle} onChange={e => setEditTitle(e.target.value)} maxLength={80} />
                </div>
                <div className="img-form-group">
                  <label className="img-label">Caption <span className="img-required">*</span></label>
                  <textarea className="img-textarea" value={editCaption} onChange={e => setEditCaption(e.target.value)} maxLength={240} rows={2} />
                </div>
                ...
                <div className="img-edit-existing-grid">
                  {editAlbum.images.map(img => (
                    <div key={img.id} className={`img-edit-existing-item${editRemoveIds.includes(img.id) ? ' marked-remove' : ''}`}>
                      <img src={img.url} alt="" />
                      <button className="img-edit-remove-img" onClick={() => toggleRemoveImage(img.id)}>
                        {editRemoveIds.includes(img.id) ? '↩' : '✕'}
                      </button>
                      {editRemoveIds.includes(img.id) && <div className="img-remove-overlay">Remove</div>}
                    </div>
                  ))}
                </div>
                ...
                >
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" width="22" height="22" style={{ color: '#2d5be3' }}>
                    <polyline points="16 16 12 12 8 16"/><line x1="12" y1="12" x2="12" y2="21"/>
                    <path d="M20.39 18.39A5 5 0 0 0 18 9h-1.26A8 8 0 1 0 3 16.3"/>
                  </svg>
