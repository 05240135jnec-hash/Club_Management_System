...
import '../../styles/advisor/blog.css';
...
export default function AdvisorBlog() {
  ...
  useEffect(() => {
    fetchBlogs();
    const handler = e => { if (!e.target.closest('.blog-menu-wrap')) setOpenMenuId(null); };
  ...
  async function handlePublish() {
    if (!title.trim() || !author.trim() || !coverFile || !content.trim()) {
      Swal.fire({ icon: 'warning', title: 'Missing fields', text: 'Title, Author, Cover Image and Content are all required.' });
      return;
    }
    setPublishing(true);
    try {
      ...
      if (res.ok) {
        ...
        setTitle(''); setAuthor(''); setContent(''); removeCover();
        fetchBlogs();
      } else {
  ...
  async function handleDelete(id, blogTitle) {
    const result = await Swal.fire({
      title: 'Delete Blog?',
      html: `Delete <strong>${blogTitle}</strong>?<br>This cannot be undone.`,
    ...
    try {
      ...
      if (res.ok) { Swal.fire({ icon: 'success', title: 'Deleted!', timer: 1500, showConfirmButton: false }); fetchBlogs(); }
    } catch {}
  }
  ...
  async function handleEditSave() {
    if (!editTitle.trim() || !editAuthor.trim()) {
      Swal.fire({ icon: 'warning', title: 'Missing fields', text: 'Title and Author are required.' });
      return;
    }
    try {
      const fd = new FormData();
  ...
  function getFiltered() {
    ...
    if (search.trim()) {
      ...
      result = result.filter(b => b.title.toLowerCase().includes(q) || b.author.toLowerCase().includes(q) || b.content?.toLowerCase().includes(q));
    }
  ...
  return (
    <AdvisorLayout title="Upload Blog" subtitle="Publish club blog posts">
      <div className="blog-page">
        ...
        <div className="blog-upload-card">
          ...
          <div className="blog-fields">
            <div className="blog-fields-row">
              <div className="blog-field-group">
                <label className="blog-label">Blog Title <span className="blog-required">*</span></label>
                <input className="blog-input" type="text" value={title} onChange={e => setTitle(e.target.value)} maxLength={150} />
              </div>
              <div className="blog-field-group">
                <label className="blog-label">Author <span className="blog-required">*</span></label>
                <input className="blog-input" type="text" value={author} onChange={e => setAuthor(e.target.value)} maxLength={80} />
              </div>
            </div>
            ...
            <div className="blog-field-group">
              ...
              {!coverPreview ? (
                ...
                >
                  ...
                  <button className="blog-browse-btn" type="button" onClick={e => { e.stopPropagation(); fileInputRef.current?.click(); }}>
                    Browse Image
                  </button>
                </div>
              ) : (
                <div className="blog-cover-preview">
                  ...
                  <div className="blog-cover-info">
                    ...
                    <div className="blog-cover-size">{coverFile ? (coverFile.size / 1024).toFixed(1) + ' KB' : ''}</div>
                  </div>
              ...
              <input ref={fileInputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif"
                style={{ display: 'none' }} onChange={e => { handleCoverFile(e.target.files[0]); e.target.value = ''; }} />
            </div>

            {/* CONTENT */}
            <div className="blog-field-group">
              <label className="blog-label">Content <span className="blog-required">*</span></label>
              <textarea className="blog-textarea" value={content} onChange={e => setContent(e.target.value)} rows={8} />
              
            </div>
          </div>

          {/* PUBLISH BUTTON - bottom right like workplan */}
          <div className="blog-publish-row">
            <button className="blog-publish-btn" onClick={handlePublish} disabled={publishing || !title.trim() || !author.trim() || !coverFile || !content.trim()}>
        ...
        <div className="blog-gallery-card">
          <div className="blog-gallery-header">
            ...
            <div className="blog-gallery-controls">
              <div className="blog-search-wrap">
                ...
                <input className="blog-search" type="text" value={search} onChange={e => setSearch(e.target.value)} />
              </div>
          ...
          {loading ? (
            ...
            <div className="blog-empty">
              ...
              <p className="blog-empty-text">{blogs.length === 0 ? 'No blogs published yet' : 'No blogs match your search'}</p>
              <p className="blog-empty-sub">{blogs.length === 0 ? 'Publish your first blog post using the form above.' : 'Try a different search term.'}</p>
            </div>
          ) : (
                  ...
                  <div className="blog-card-body">
                    <div className="blog-card-title-row">
                      ...
                      <div className="blog-menu-wrap" onClick={e => e.stopPropagation()}>
                        <button className="blog-menu-btn" onClick={() => setOpenMenuId(openMenuId === b.id ? null : b.id)}>
                        ...
                        {openMenuId === b.id && (
                          <div className="blog-dropdown">
                            <button className="blog-dd-item view" onClick={() => { setViewBlog(b); setOpenMenuId(null); }}>
                            ...
                            <button className="blog-dd-item delete" onClick={() => handleDelete(b.id, b.title)}>
                              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" width="13" height="13">
                    ...
                    <div className="blog-card-author-row">
                      <div className="blog-card-avatar">{getInitials(b.author)}</div>
        ...
        {/* ── VIEW MODAL — matches student view ── */}
              ...
              <div className="blog-view-body">
                <div className="blog-view-content">
                  ...
                  <div className="blog-view-author-row">
                    <div className="blog-view-avatar">{getInitials(viewBlog.author)}</div>
                    <span className="blog-view-author-name">{viewBlog.author}</span>
          ...
          <div className="blog-modal-backdrop" onClick={() => setEditBlog(null)}>
            <div className="blog-edit-modal" onClick={e => e.stopPropagation()}>
              ...
              <div className="blog-edit-cover-section">
                {editCoverPreview ? (
                  <div className="blog-edit-cover-overlay" onClick={() => editCoverInputRef.current?.click()}>
                  ...
                  <div className="blog-edit-cover-placeholder" onClick={() => editCoverInputRef.current?.click()}>
                ...
                <input ref={editCoverInputRef} type="file" accept="image/jpeg,image/png,image/webp,image/gif"
                  style={{display:'none'}} onChange={e => { handleEditCoverFile(e.target.files[0]); e.target.value=''; }} />
              </div>

              <div className="blog-edit-body">
                <div className="blog-field-group">
                  <label className="blog-label">Blog Title <span className="blog-required">*</span></label>
                  <input className="blog-input" type="text" value={editTitle} onChange={e => setEditTitle(e.target.value)} maxLength={150} />
                </div>
                <div className="blog-field-group">
                  <label className="blog-label">Author <span className="blog-required">*</span></label>
                  <input className="blog-input" type="text" value={editAuthor} onChange={e => setEditAuthor(e.target.value)} maxLength={80} />
                </div>
                <div className="blog-field-group">
                  <label className="blog-label">Content <span className="blog-required">*</span></label>
                  <textarea className="blog-textarea" value={editContent} onChange={e => setEditContent(e.target.value)} rows={6} />
                </div>
              </div>
