...
function Dashboard() {
  ...
  const handleEditCategory = async () => {
    ...
    const exists = categories.some(c => c.name.toLowerCase() === name.toLowerCase() && c.id !== editCategory.id);
    if (exists) { setEditError('This category already exists.'); return; }

    setEditLoading(true);
    try {
      const token = localStorage.getItem('token');
      const res = await axios.put(`/api/categories/${editCategory.id}`, { name }, {
        headers: { Authorization: `Bearer ${token}` }
      });
      await fetchAll();
          ...
          return (
            ...
            >
              ...
              <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                ...
                <div className="dots-menu-wrap">
                  ...
                  {openMenu === cat.id && (
                    <div className="dots-dropdown" onClick={e => e.stopPropagation()}>
                      ...
                      <button className="dots-dd-item delete" onClick={() => handleDelete(cat.id)}>
                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                {/* CHEVRON */}
                <button className="chevron-btn" onClick={e => { e.stopPropagation(); setOpenCard(openCard === cat.id ? null : cat.id); }}>
                  <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              ...
              {openCard === cat.id && (
                <div className="club-dropdown visible" onClick={e => e.stopPropagation()}>
                  <div className="dropdown-header">
                    ...
                    <span className="dropdown-header-count">{catClubs.length} club{catClubs.length !== 1 ? 's' : ''}</span>
                  </div>
                  {catClubs.length > 0 ? (
          ...
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <input
                ...
                onKeyDown={e => { if (e.key === 'Enter') handleAddCategory(); if (e.key === 'Escape') setShowAddModal(false); }}
                autoFocus
              />
          ...
          <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <input
                ...
                onKeyDown={e => { if (e.key === 'Enter') handleEditCategory(); if (e.key === 'Escape') setShowEditModal(false); }}
                autoFocus
              />
