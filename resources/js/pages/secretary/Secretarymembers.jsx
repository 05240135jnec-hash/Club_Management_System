...
import '../../styles/secretary/members.css';
...
function ThreeDotMenu({ items }) {
  ...
  const handleOpen = (e) => {
    ...
    const dropWidth = 180;
    const margin = 8;
    // Use page body bounds instead of window width
    const page = document.querySelector('.sl-page-body') || document.body;
    const pageRect = page.getBoundingClientRect();
    const maxRight = pageRect.right - margin;
    let left = rect.right - dropWidth;
    if (left < pageRect.left + margin) left = pageRect.left + margin;
    if (left + dropWidth > maxRight) left = maxRight - dropWidth;
    setPos({
      top:  rect.bottom + 4,
      left: left,
    });
    ...
    <>
      ...
      {open && createPortal(
        <div
          ...
          style={{ position: 'fixed', top: pos.top, left: pos.left, right: 'auto', zIndex: 99999 }}
        >
          {items.map((item, i) => (
            <button
              key={i}
...
export default function SecretaryMembers() {
  ...
  const handleRemove = async (id, name) => {
    ...
    if (member?.role === 'secretary') {
      Swal.fire({ icon: 'warning', title: 'Cannot Remove Member', text: 'This member is currently assigned as a Secretary. Please remove their Secretary role first.', confirmButtonColor: '#2d5be3' });
      return;
    }
    const result = await Swal.fire({ title: `Remove ${name}?`, text: `${name} will be moved to Inactive Members.`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#e53e3e', cancelButtonColor: '#6b7f93', confirmButtonText: 'Yes, Remove', cancelButtonText: 'Cancel', customClass: { popup: 'swal-custom-popup', title: 'swal-custom-title' }, focusCancel: true });
  ...
  const handleBulkRemove = async () => {
    ...
    if (hasSecretary) { Swal.fire({ icon: 'warning', title: 'Cannot Remove', text: 'One or more selected members are Secretary. Please remove their role first.', confirmButtonColor: '#2d5be3' }); return; }
  ...
  const handlePermDelete = async (id, name) => {
    const result = await Swal.fire({ title: `Permanently Delete ${name}?`, text: `This cannot be undone.`, icon: 'warning', showCancelButton: true, confirmButtonColor: '#e53e3e', cancelButtonColor: '#6b7f93', confirmButtonText: 'Yes, Delete Permanently', cancelButtonText: 'Cancel', customClass: { popup: 'swal-custom-popup', title: 'swal-custom-title' }, focusCancel: true });
    if (!result.isConfirmed) return;
    try {
  ...
  if (loading) return <SecretaryLayout title="Members" subtitle="View and manage club members"><div className="mem-loading">Loading members...</div></SecretaryLayout>;

  return (
    <SecretaryLayout title="Members" subtitle="View and manage club members">
        ...
        <>
          ...
          <div className="mem-table-wrap">
            <table className="mem-table">
              <thead>
                <tr>
                  <th className="col-check"><input type="checkbox" className="mem-check" checked={allChecked} ref={el => { if (el) el.indeterminate = someChecked; }} onChange={toggleAll} /></th>
                  <th>MEMBER</th>
                  <th>STUDENT ID</th>
                  <th>COURSE</th>
                ...
                {filteredActive.length === 0 ? (
                  ...
                  return (
                    <tr key={m.id} className={selectedIds.includes(m.id) ? 'row-selected' : ''}>
                      ...
                      <td style={{ lineHeight: 0, verticalAlign: 'middle', width: 44 }}>
                        <ThreeDotMenu items={[
