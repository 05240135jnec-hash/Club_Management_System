...
import '../../styles/secretary/attendance.css';
...
export default function SecretaryAttendance() {
  ...
  // AUTO-SAVE: fires on every checkbox tick, no reload, no blink
  ...
  return (
    <SecretaryLayout title="Attendance" subtitle="Track and manage club session attendance">
          ...
          <div className="att-modal att-profile-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="att-profile-body">
              ...
              <div className="att-profile-rows">
                <div className="att-profile-row"><span className="att-profile-label">Student ID</span><span className="att-profile-val">{profileMember.student_id || '—'}</span></div>
                <div className="att-profile-row"><span className="att-profile-label">Email</span><span className="att-profile-val">{profileMember.email || '—'}</span></div>
                <div className="att-profile-row"><span className="att-profile-label">Course</span><span className="att-profile-val">{profileMember.course || '—'}</span></div>
                <div className="att-profile-row"><span className="att-profile-label">Year</span><span className="att-profile-val">{yearLabel(profileMember.year)}</span></div>
                <div className="att-profile-row">
                  <span className="att-profile-label">Attendance</span>
                  <span className={`att-profile-val ${profileMember.overall_pct < 75 ? 'att-pct-low' : 'att-pct-ok'}`}>{profileMember.overall_pct}%</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="att-modal-backdrop" onClick={() => { setNewModal(false); setFormName(''); setFormDate(''); }}>
          <div className="att-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="att-modal-footer">
              <button className="att-btn-cancel-form" onClick={() => { setNewModal(false); setFormName(''); setFormDate(''); }}>Cancel</button>
              <button className="att-btn-create" onClick={createSession}>Create Session</button>
            </div>
          </div>
        </div>, document.body
      )}
      ...
      <div className="att-page">
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, gap: 16 }}>
          ...
          <button
            ...
            onClick={() => { setFormName(''); setFormDate(''); setNewModal(true); }}
