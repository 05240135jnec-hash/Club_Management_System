...
import '../../styles/secretary/report.css';
...
export default function SecretaryReports() {
  ...
  const [editForm, setEditForm]         = useState({ title: '', type: '', sent_to: 'advisor' });
  const [form, setForm]                 = useState({ title: '', type: '', sent_to: 'advisor' });
  ...
  async function fetchReports() {
    ...
    try {
      ...
      // fetch feedback counts for audit reports
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const res  = await fetch(`${API}/reports/secretary`, {
        method: 'POST',
        headers: { Authorization: `Bearer ${getToken()}` },
        body: fd,
      });
      ...
      if (res.ok) {
        ...
        setForm({ title: '', type: '', sent_to: 'advisor' });
        removeFile();
        fetchReports();
      } else {
  ...
  function openEdit(r) {
    setOpenMenuId(null);
    setEditReport(r);
    setEditForm({ title: r.title, type: r.type, sent_to: r.sent_to || 'advisor' });
    setEditModal(true);
  }
    ...
    try {
      ...
      const res = await fetch(`${API}/reports/${editReport.id}`, {
        ...
        body: JSON.stringify({
          ...
          type:    editForm.type,
          sent_to: isEditAudit ? 'advisor' : editForm.sent_to,
        }),
      });
      const data = await res.json();
      if (res.ok) {
  ...
  function filteredReports() {
    ...
    if (activeTab === 'me')      return reports.filter(r => r.sent_to === 'me');
    if (activeTab === 'advisor') return reports.filter(r => r.sent_to === 'advisor' && !r.is_audit);
    if (activeTab === 'audit')   return reports.filter(r => r.is_audit);
    return reports;
  }

  const SEND_TO_HINTS = {
    advisor: 'Submitted to the Club Advisor for review',
  ...
  const tabs = [
    ...
    { key: 'advisor', label: 'Submitted to Advisor' },
  ...
  return (
    <SecretaryLayout title="Upload Reports" subtitle="Submit club reports to advisor">
        ...
        <div className="rpt-card">
          ...
          {!selectedFile ? (
            ...
            >
              ...
              <div className="rpt-drop-sub">PDF, Word, or Excel files</div>
                      ...
                      >
                        ...
                        <td>
                          {r.is_audit ? (
                            ...
                            <span className={`rpt-badge ${r.sent_to === 'advisor' ? 'dsa' : 'me'}`}>
                              {r.sent_to === 'advisor' ? 'Advisor' : 'Only Me'}
                            </span>
                          )}
                        </td>
                        {activeTab === 'audit' && (
