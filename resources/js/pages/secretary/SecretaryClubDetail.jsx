...
import '../../styles/secretary/secretaryclubdetail.css';

const TEXT = { color: '#1a1a2e' };

export default function SecretaryClubDetails() {
  ...
  useEffect(() => { fetchMyClub(); fetchCategories(); }, []);
  ...
  const openEdit = () => {
    ...
    setForm({ name:club.name||'', category:club.category||'', aim:club.aim||'', description:club.description||'', objectives:objectives.join('\n') });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setEditMode(false); setCoverFile(null); setCoverPreview(null);
      showToast();
    } catch (err) {
  ...
  if (loading) return <SecretaryLayout title="Club Details" subtitle="View and manage your club information"><div className="acd-loading">Loading...</div></SecretaryLayout>;
  if (!club)   return <SecretaryLayout title="Club Details" subtitle="View and manage your club information"><div className="acd-empty">No club found.</div></SecretaryLayout>;
  ...
  return (
    <SecretaryLayout title="Club Details" subtitle="View and manage your club information">
      ...
      {!editMode ? (
        <div className="acd-card">
          ...
          <div className="acd-body">
            ...
            <div className="acd-divider" />
            <div className="acd-section">
              <div className="acd-section-label">Objectives of <strong style={TEXT}>{club.name}</strong></div>
              {objectives.length > 0 ? (
                <ul className="acd-objectives">{objectives.map((obj, i) => <li key={i} style={TEXT}>{obj}</li>)}</ul>
          ...
          <div className="acd-stats">
            ...
            <div className="acd-stat">
              ...
              {coAdvisors.length > 0 ? (
                <div style={{display:'flex',flexDirection:'column',gap:2}}>
                  {coAdvisors.map(ca => <span key={ca.id} className="acd-stat-val" style={TEXT}>{ca.name}</span>)}
                </div>
            ...
            <div className="acd-stat">
              ...
              {secretaries.length > 0 ? (
                <div style={{display:'flex',flexDirection:'column',gap:2}}>
                  {secretaries.map(s => <span key={s.id} className="acd-stat-val" style={TEXT}>{s.name}</span>)}
                </div>
              ) : <span className="acd-stat-val" style={TEXT}>Not assigned yet</span>}
            </div>
          </div>
        ...
        <div className="acd-card">
          ...
          <div className="acd-form-grid">
            ...
            <div className="acd-form-group">
              ...
              <input className="acd-form-input" type="text" value={form.name} onChange={e => setForm(p=>({...p,name:e.target.value}))} />
            </div>
            <div className="acd-form-group">
              <label className="acd-form-label">Category</label>
              <select className="acd-form-input acd-form-select" value={form.category} onChange={e => setForm(p=>({...p,category:e.target.value}))}>
            ...
            <div className="acd-form-group acd-form-full">
              ...
              <input className="acd-form-input" type="text" value={form.aim} onChange={e => setForm(p=>({...p,aim:e.target.value}))} />
            </div>
            <div className="acd-form-group acd-form-full">
              <label className="acd-form-label">Description</label>
              <textarea className="acd-form-input acd-form-textarea" value={form.description} onChange={e => setForm(p=>({...p,description:e.target.value}))} rows={4} />
            </div>
            <div className="acd-form-group acd-form-full">
              <label className="acd-form-label">Objectives <span className="acd-label-hint">(one per line)</span></label>
              <textarea className="acd-form-input acd-form-textarea" value={form.objectives} onChange={e => setForm(p=>({...p,objectives:e.target.value}))} rows={5} />
            </div>
          </div>
