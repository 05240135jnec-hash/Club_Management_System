...
export default function SecretaryDashboard() {
  ...
  if (viewItem) {
    ...
    return (
      <SecretaryLayout title="Dashboard" subtitle="Welcome back — here's your club at a glance">
          ...
          <div className="ann-view-card">
            ...
            <div className="ann-view-authors">
              {(viewItem.posted_by || viewItem.created_by) && (
                <span>Posted by: <strong>{viewItem.posted_by || viewItem.created_by}</strong></span>
              )}
              {(viewItem.last_edited_by || viewItem.updated_by) && (
                <span>Last edited by: <strong>{viewItem.last_edited_by || viewItem.updated_by}</strong></span>
              )}
            </div>
            ...
            {viewItem.attachment && !isImageFile(viewItem.attachment) && (
              <a href={viewItem.attachment} target="_blank" rel="noreferrer" className="ann-view-attachment-btn">
    ...
    <SecretaryLayout title="Dashboard" subtitle="Welcome back — here's your club at a glance">
      ...
      <div className="adb-card adb-ann-full">
        <div className="adb-card-header">
          ...
          <button className="adb-see-all-btn" onClick={() => navigate('/secretary/announcements')}>
            See all →
          </button>
        </div>
                ...
                <div key={ann.id} className="adb-ann-item" onClick={() => setViewItem(ann)}>
                  <div className="adb-ann-body">
                    ...
                    <div className="adb-ann-desc">
                      {ann.content?.length > 160 ? ann.content.slice(0, 160) + '...' : ann.content}
                    </div>
                    <div className="adb-ann-footer">
                      ...
                      <div className="adb-ann-meta">
                        <span className="adb-ann-recipient">{getRecipientLabel(ann.recipients)}</span>
                        <span className="adb-ann-date">{ann.date_sent || formatDate(ann.created_at)}</span>
                        {(ann.posted_by || ann.created_by) && (
                          <span className="adb-ann-author">Posted by: {ann.posted_by || ann.created_by}</span>
                        )}
                        {ann.attachment && (
                          <span className="adb-ann-attach-tag"><PaperclipIcon />Attachment</span>
                        )}
                      </div>
                    </div>
                  </div>
