...
let roomsData = [
  ...
  { id: 7, floor_number: 1, room_number: '7', room_type: 'executive', total_beds: 2, status: 'available', booking_status: 'available', booked_beds: 0, free_beds: 2, bed_bookings: [] },
...
function renderMainPage() {
  ...
  const html = `
    <div class="stats-grid">
      <div class="stat-card" style="--stat-color:#172D3D"><div class="stat-label">Total Rooms</div><div class="stat-value">${totalRooms}</div></div>
      <div class="stat-card" style="--stat-color:#1d4ed8"><div class="stat-label">Total Beds</div><div class="stat-value">${totalBeds}</div></div>
      <div class="stat-card" style="--stat-color:#16a34a"><div class="stat-label">Available</div><div class="stat-value">${available}</div></div>
      <div class="stat-card" style="--stat-color:#d97706"><div class="stat-label">Partial</div><div class="stat-value">${partial}</div></div>
      <div class="stat-card" style="--stat-color:#dc2626"><div class="stat-label">Occupied</div><div class="stat-value">${occupied}</div></div>
    </div>
        ...
        <div class="floor-section">
          ...
          <div class="rooms-grid">
            ${floorRooms.sort((a,b) => parseInt(a.room_number) - parseInt(b.room_number)).map(room => {
              ...
              return `
                <div class="room-card ${isClickable ? 'clickable' : ''}" style="--border-color:${ss.border}" onclick="${isClickable ? `openBedDetailsModal(${room.id})` : 'return false'}">
                  ...
                  <div class="room-beds">${isPartial ? `<strong>${room.booked_beds || 0}</strong> / ${room.total_beds} beds booked` : `${room.total_beds} bed${room.total_beds > 1 ? 's' : ''}`}</div>
                  ...
                  ${isClickable ? `<div class="click-hint">Click to view bed details</div>` : ''}
                </div>
              `;
            }).join('')}
          </div>
...
function submitAddRooms() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    roomsData.push({
      ...
      booked_beds: 0,
      free_beds: parseInt(addForm.total_beds),
      bed_bookings: []
    });
  }
...
function saveEditRoom() {
  if (currentEditRoom) {
    ...
    currentEditRoom.free_beds = currentEditRoom.total_beds - (currentEditRoom.booked_beds || 0);
...
function navigateTo(page, element) {
  ...
  const titles = {
    dashboard: ['Dashboard', 'Overview of your guesthouse'],
    details: ['Guesthouse Details', 'Manage your guesthouse information'],
    rooms: ['Manage Rooms', 'Add and manage your guesthouse rooms'],
    bookings: ['Bookings', 'View and manage guest bookings']
  };
  ...
  if (page === 'rooms') {
    ...
    document.getElementById('page-body').innerHTML = `<div class="empty-state"><p>${title} — Coming Soon</p></div>`;
  }
}
