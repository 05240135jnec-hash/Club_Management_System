...
var CLUB_DASHBOARDS = {
  ...
  "Media Club":                   "../html/dashboard.html",
...
/* =============================================
   NAVBAR CATEGORIES — desktop dropdowns
============================================= */
...
/* =============================================
   FORCE CLEAR SEARCH — beats browser autocomplete
   Sets readonly first, then removes it after a tick.
   Chrome/Edge will not autofill readonly inputs.
============================================= */
function forceBlankSearch() {
  var ids = ['nav-search-input', 'nav-mobile-search-input'];
  ...
  /* Remove readonly after browser has finished autofill attempt */
  setTimeout(function() {
    ids.forEach(function(id) {
      var el = document.getElementById(id);
      if (!el) return;
...
/* Run again after full page load to catch late autofill */
window.addEventListener('load', forceBlankSearch);
/* One more pass 500ms after load — covers slowest autofill engines */
window.addEventListener('load', function() {
  setTimeout(forceBlankSearch, 500);
});